<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyMatchIsVisible(scope.row.matchInfoEntity)" type="text" size="small" @click="applyMatchHandle(scope.row.matchInfoEntity.matchId)">评奖</el-button>
    ...
    <award-add-or-update v-if="awardVisible" ref="award" @refreshDataList="applyMatchRef"></award-add-or-update>
  </div>
</template>
      ...
      applyMatchIsVisible (item) {
        if (this.isAuth('innovate:project:apply:match')) {
          if (item.projectMatchApplyStatus !== null || item.projectMatchApplyStatus !== '') {
            if (item.projectMatchApplyStatus === 5) {
