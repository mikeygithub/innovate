    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyIsVisible(scope.row.matchInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.matchInfoEntity.matchId)">分配评委组</el-button>
          <el-button v-if="reApplyIsVisible(scope.row.matchInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.matchInfoEntity.matchId)">重新分配评委组</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      canUpdateVisible (item) {
        this.$http({
          url: this.$http.adornUrl(`/innovate/match/apply/update/list`),
          method: 'post',
